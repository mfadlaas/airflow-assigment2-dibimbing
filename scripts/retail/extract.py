def extract_logic(item, **kwargs):
    ...
    with postgres_hook.connect() as conn:
        ...
        ingest_type  = kwargs['var']['json'].get("retail_ingest_type", {}).get(item['table'], "full")
    ...
    with mysql_hook.connect() as conn:
        ...
        os.makedirs("data/retail", exist_ok=True)
        df.to_parquet(f"data/retail/{item['table']}.parquet", index=False)
